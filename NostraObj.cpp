...
NOSTRAOBJ* InitNostra(DOUBLEBUFFER* db)
{
	...
	Temp = (NOSTRAOBJ*)malloc(sizeof(NOSTRAOBJ));
	...
	//이미지 로딩
	Temp->Sprite[LEFT_SWIM] = InitSprite(L"./Image/Fish/NOSTRADAMUS/NOSTRADAMUS.bmp", 80, 80, 0, 10, Temp->InvisibleColor);
	Temp->Sprite[RIGHT_SWIM] = InitSprite(L"./Image/Fish/NOSTRADAMUS/NOSTRADAMUS2.bmp", 80, 80, 0, 10, Temp->InvisibleColor);
	Temp->Sprite[R2L_TURN] = InitSprite(L"./Image/Fish/NOSTRADAMUS/NOSTRADAMUS2.bmp", 80, 80, 80, 10, Temp->InvisibleColor, TRUE);
	Temp->Sprite[L2R_TURN] = InitSprite(L"./Image/Fish/NOSTRADAMUS/NOSTRADAMUS.bmp", 80, 80, 80, 10, Temp->InvisibleColor);
...
// 다음 프레임으로 스프라이트 업데이트
void NextFrameSprite(NOSTRAOBJ* pNostra, int state, int frameRate)
{
	...
	if (elapsedTime > desiredFPS)
	{
		...
		pNostra->Sprite[state]->nCurrentFrame++;
		if (pNostra->Sprite[state]->nCurrentFrame > pNostra->Sprite[state]->nLastFrame)
			pNostra->Sprite[state]->nCurrentFrame = 0;
	}
}
